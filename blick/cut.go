...
func main() {
	jpgFiles, err := filepath.Glob("colors/*_*_*.jpg")
	if err != nil {
		log.Fatal(err)
	}
	...
	for i, inPath := range jpgFiles {
		...
		hexString, err = crop(inPath, inPath+".2.png", image.Point{450, 230})
		if err != nil {
			log.Fatal(err)
		}
...
func crop(inPath string, outPath string, tlPoint image.Point) (hexString string, err error) {
	...
	cImg, err := cutter.Crop(img, cutter.Config{
		Height: 120,            // height in pixel or Y ratio(see Ratio Option below)
		Width:  120,            // width in pixel or X ratio
